...
import json
import os
import psutil  # Pour vérifier les processus en cours
...
def reponse_par_llama(question):
    """Retourne une réponse à partir du modèle Ollama ou du cache."""
    ...
    try:
        ...
        sauvegarder_cache()  # Sauvegarde uniquement après une nouvelle réponse
        return reponse
...
def lancer_ollama():
    """Lance Ollama si ce n'est pas déjà en cours."""
    # Vérifie si le processus Ollama est déjà en cours
    for proc in psutil.process_iter(['pid', 'name']):
        if 'ollama' in proc.info['name'].lower():
            logging.info("Ollama est déjà en cours d'exécution.")
            return
